...
def print_pooling_info(name, filter_size, stride, pad, shape_before, shape_after, time):
    ...
    print('%s(%d × %d, stride=%d, pad=%d) (%d x %d x %d) -> (%d x %d x %d) (cost=%d): %.6f[sec]' %
          (name, filter_size, filter_size, stride, pad, shape_before[2], shape_before[3], shape_before[1],
...
class Box():
    ...
    def int_left_top(self):
        ...
        return (int(self.x - half_width), int(self.y - half_height))
...
def multi_box_intersection(a, b):
    ...
    zeros = Variable(xp.zeros(w.shape, dtype=w.data.dtype))
    zeros.to_gpu()
...
def random_hsv_image(bgr_image, delta_hue, delta_sat_scale, delta_val_scale):
    hsv_image = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2HSV).astype(xp.float32)

    # hue
    hsv_image[:, :, 0] += int((xp.random.rand() * delta_hue * 2 - delta_hue) * 255)

    # sat
    sat_scale = 1 + xp.random.rand() * delta_sat_scale * 2 - delta_sat_scale
    hsv_image[:, :, 1] *= sat_scale

    # val
    val_scale = 1 + xp.random.rand() * delta_val_scale * 2 - delta_val_scale
    hsv_image[:, :, 2] *= val_scale

    hsv_image[hsv_image < 0] = 0
    hsv_image[hsv_image > 255] = 255
    hsv_image = hsv_image.astype(xp.uint8)
