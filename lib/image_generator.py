...
def delete_pad(image):
    ...
    mask = xp.argwhere(image[:, :, 3] > 128)  # alphaチャンネルの条件、!= 0 や == 255に調整できる
...
def random_sampling(image, h, w):
    orig_h, orig_w = image.shape[:2]
    y = xp.random.randint(orig_h - h + 1)
    x = xp.random.randint(orig_w - w + 1)
    return image[y:y + h, x:x + w]


# 画像をランダムに回転、スケールしてから返す
def random_rotate_scale_image(image, min_scale, max_scale, rand_angle):
    image = rotate_image(image, xp.random.randint(rand_angle * 2) - rand_angle)
    image = scale_image(image, min_scale + xp.random.rand() * (max_scale - min_scale))  # 1 ~ 3倍
...
def random_overlay_image(src_image, overlay_image, minimum_crop):
    ...
    y = int(xp.random.randint(src_h - scale_item_h) - shift_item_h)
    x = int(xp.random.randint(src_w - scale_item_w) - shift_item_w)
    image = overlay(src_image, overlay_image, x, y)
    bbox = (
    (np.maximum(x, 0), xp.maximum(y, 0)), (np.minimum(x + overlay_w, src_w - 1), np.minimum(y + overlay_h, src_h - 1)))
...
class ImageGenerator():
    def __init__(self, item_path, background_path):
        ...
        for item_file in self.item_files:
            ...
            center = xp.maximum(image.shape[0], image.shape[1])
            pixels = xp.zeros((center * 2, center * 2, image.shape[2]))
            y = int(center - image.shape[0] / 2)
            x = int(center - image.shape[1] / 2)
            pixels[y:y + image.shape[0], x:x + image.shape[1], :] = image
            self.items.append(pixels.astype(xp.uint8))
    ...
    def generate_random_animation(self, loop, bg_index, crop_width, crop_height, min_item_scale, max_item_scale):
        ...
        for i in range(loop):
            ...
            item = scale_image(item, min_item_scale + xp.random.rand() * (max_item_scale - min_item_scale))
            orig_item = item
            item_height, item_width, _ = item.shape
            edges = [-item_width, -item_height, bg_width, bg_height]
            r = xp.random.randint(2)
            rand1 = xp.random.randint(edges[r + 2] - edges[r]) + edges[r]
            ...
            animate_frames = xp.random.randint(30) + 50
            angle = xp.random.rand() * 10 - 5
            rotate_cnt = 0
            total_angle = 0
    ...
    def generate_samples(self, n_samples, n_items, crop_width, crop_height, min_item_scale, max_item_scale, rand_angle,
        ...
        for i in range(n_samples):
            bg = self.bgs[xp.random.randint(len(self.bgs))]
            sample_image = random_sampling(bg, crop_height, crop_width)

            ground_truths = []
            boxes = []
            for j in range(xp.random.randint(n_items) + 1):
                class_id = xp.random.randint(len(self.labels))
                item = self.items[class_id]
                ...
                if maximum_iou(box, boxes) < 0.3:
                    boxes.append(box)
                    one_hot_label = xp.zeros(len(self.labels))
            ...
            sample_image = xp.asarray(sample_image, dtype=xp.float32) / 255.0
